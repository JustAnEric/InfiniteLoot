...
def GetUsername(token):
    ...
    info = get('https://discord.com/api/v6/users/@me', headers=headers).json()
...
def BillingCheck(token):
    ...
    info = get('https://discord.com/api/v6/users/@me/billing/payment-sources', headers=headers).json()
    print(info)
    if len(info) > 0:
        billing_info = []
...
def NitroCheck(token):
    ...
    has_nitro = False
    res = get('https://discord.com/api/v6/users/@me/billing/subscriptions', headers=headers)
    nitro_data = res.json()
